...
async def _write_or_supersede_fact(
    ...
    *,
    ...
    """Write a fact unless an identical (section, field, value) is already current.

    When a current fact exists with a *different* value, supersede it
    (the new fact gets the source_event_id; the old fact's
    ``superseded_by`` and ``valid_to`` are stamped). Returns True when
    ...
    current = (
        await session.execute(
            ...
            {"pid": property_id, "section": section, "field": field},
        )
    ).first()
    ...
    insert_result = await session.execute(
        text(
            """
            INSERT INTO facts (property_id, section, field, value,
                               source_event_id, confidence, valid_from)
            VALUES (:pid, :section, :field, :value, :eid, :conf, now())
            RETURNING id
            """
        ),
        {
...
async def extract_bank_facts(
    ...
    *,
    ...
    No-op when ``property_id`` is None (the event is unrouted). The
    Phase 8 Step 9 admin endpoints surface unrouted events for human
    triage; until they're routed, no per-property fact is appropriate.
    """
    ...
    formatted = f"{_format_amount(amount)} on {valuta}" if valuta else _format_amount(amount)

    written = 0
    if kategorie == "miete" and typ == "CREDIT":
        written += int(
    ...
    elif kategorie == "kaution" and typ == "CREDIT":
        written += int(
            await _write_or_supersede_fact(
                session,
                property_id=property_id,
...
async def extract_invoice_facts(
    ...
    *,
    ...
    Invoices in Buena's archive are property-management bills (paid by
    the WEG, not a single unit), so most have no ``property_id`` even
    after routing. When the property *is* known, we record the latest
    invoice + ensure a ``serviced_by`` edge to the contractor.
    """
...
async def stamp_processed(
    ...
    *,
    ...
    """Mark the structured event as processed_at + write the resolved property_id."""
    await session.execute(
        text(
            """
            UPDATE events
        ...
        {"id": event_id, "pid": property_id, "ts": received_at},
    )
